...
p.innerText = "This is a list of users";
header.appendChild(p);


// Wrapper & its elements and pagination element:
const mainwrapper = document.createElement("main");
document.body.appendChild(mainwrapper);
const list = document.createElement("div");
mainwrapper.appendChild(list);
list.setAttribute("class", "table-responsive");
list.setAttribute("id", "list")
const pageNum = document.createElement("div");
const mainElement = document.querySelector("main");
mainElement.appendChild(pageNum);
pageNum.setAttribute("class", "pageNumbers d-flex justify-content-center");
pageNum.setAttribute("id", "buttons");
...
// creating table and table rows and table data 
...
const tableHeadElement = document.querySelector("thead")
const row = document.createElement("tr");
row.setAttribute("class", "heading");
tableHeadElement.appendChild(row);
// table heading
const headingArr = ["id", "name", "email"];
for (let j = 0; j < headingArr.length; j++) {
  const tableRow = document.querySelector(".heading");
  const tabletitle = document.createElement("th");
  tabletitle.setAttribute("class", headingArr[j]);
  tabletitle.innerText = headingArr[j].toUpperCase();
  tableRow.appendChild(tabletitle);
}

// Create Table Body
const tableBody = document.createElement("tbody");
tableBody.setAttribute("id", "wrappe");
tableElement.appendChild(tableBody);


let currentPage = 1;
let rows = 10;
const wrapper = document.getElementById("wrappe");

function displayTable(data, wrap, rowsPerPage, page) {
  wrap.innerHTML = "";
  page--;
  ...
  for (i = 0; i < paginatedItems.length; i++) {
    ...
    for (let k = 0; k < keys.length; k++) {
      ...
      const cItem = paginatedItems[i];
      td.innerText = cItem[keys[k]];
      tr.appendChild(td);
    }
    wrap.appendChild(tr);
  }
...
const paginationBtn = document.querySelector(".page-num");
const bounder = document.querySelector("#buttons");
function setupPagination(Data, wrap, rowsPerPage, page) {
  wrap.innerHTML = "";
  let btn = document.createElement("button");
  btn.classList.add("prev");
  btn.innerText = "Prev";
  btn.addEventListener("click", () => {
    displayTable(Data, wrapper, rows, currentPage - 1)
  })
  wrap.appendChild(btn);
  let pageNum = Math.ceil(Data.length / rowsPerPage);
  for (let k = 1; k < pageNum + 1; k++) {
    let pageBtn = pagiBtn(k, Data);
...
function pagiBtn(page, Data) {
  ...
  btn.addEventListener("click", function () {
    currentPage = page;
    displayTable(Data, wrapper, rows, currentPage);
  });
  return btn;
}

displayTable(Data, wrapper, rows, currentPage);
setupPagination(Data, bounder, rows, currentPage);
